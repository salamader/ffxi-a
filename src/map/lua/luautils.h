...
namespace luautils
{
    ...
    int32 SendUnknown0x39Packet(lua_State*);                                    // Temporary solution for geysers in Dangruf Wadi
    ...
    int32 GetRegionOwner(lua_State*);										    // Know the country that owns the current region
	...
	int32 SpawnNPC(lua_State*);													// Spawn Mob By NPC Id - NMs, BCNM...
	int32 GetPlayerByName(lua_State*);											// Gets Player ref from a name supplied
	int32 GetMobAction(lua_State*);												// Get Mobs current action
	int32 VanadielTOTD(lua_State*);												// Current playing time of day
	int32 VanadielHour(lua_State*);												// Current Vanadiel hour
	int32 VanadielMinute(lua_State*);											// Current Vanadiel minute
    ...
    int32 GetTextIDVariable(uint16 ZoneID, const char* variable);               // load the value of the specified zone TextID
