...
CZoneInPacket::CZoneInPacket(CCharEntity * PChar, int16 csid) 
{
  ...
  //WBUFL(data,(0x70)-4) = PChar->loc.zone->GetWeatherChangeTime();
	...
	if (PChar->getZone() == 0) 
	{
        ...
        WBUFB(data,(0xAC)-4) = csid > 0 ? 0x01 : 0x00;
	...
	if (PChar->is_dead == 1) 
    {
		...
		ShowDebug(CL_BG_GREEN" DEATH STATE IS DEAD TIME %u\n"CL_RESET,PChar->m_DeathTimestamp);
		WBUFL(data,(0xA4)-4) =  PChar->m_DeathTimestamp;
	}
