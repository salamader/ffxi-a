...
function onTrade(player,npc,trade)
	...
	if(ClearChip == true and count == 1) then
		...
		player:setVar("LargeApparatusDem",1);
	end
end;
...
function onTrigger(player,npc)
	if(player:getVar("LargeApparatusDem") < 1) then 
		player:startEvent(0x00A3); -- Something to due with having the keyitem missing chip
	end
	if(player:getVar("LargeApparatusDem") == 1 and player:getVar("TuLiaRegistration") == 1) then 
		if(math.random(0,100) <= 5) then
			player:setVar("LargeApparatusDem",0); -- Clear Chip broke
		end
